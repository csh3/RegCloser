...
with open(args.p,'r') as fin:
	...
	for line in fin:
		...
		if len(record) == 7:
			if record[6] == '1' or record[6] == '3':
				maxExtendLen = max(maxExtendLen,int(record[3])+args.ml*int(record[4])+200)
	maxReadNum=maxExtendLen*2*args.rc/100
...
if(args.s=="LocalAssembly" or progress_running == 1):
	...
	if args.w:
		os.system("cd %s; python %s/LocalAssembly.py -w -t %d -l %d -rc %d -rn %d -S %f -qc %f -r1 %f -r2 %f -ma %d -mm %d -gc %d -ms %d -ho %d -mT %d -mA %d -mM %d -mG %d -mS %d -HO %d -o %s > LocalAssembly.log 2>&1"%(args.d, path, args.t, args.l, args.rc, maxReadNum, args.S, args.qc, args.r1, args.r2, args.ma, args.mm, args.gc, args.ms, args.ho, args.mT, args.mA, args.mM, args.mG, args.mS, args.HO, args.o))
	else:
