...
#-------------------------------------------------CONTIG EXTENSION USING UNMAPPED PAIRS STORED IN $SET
    ...
    my $outfileTig
        = "$base_name/intermediate_results/"
        . $base_name
        . ".extendedcontigs.fasta";
    my $outfileEvidence
        = "$base_name/intermediate_results/"
        . $base_name
        . ".extension_evidence.txt";

    open( TIG, ">$outfileTig" )
        || die "Can't write to $outfileTig -- fatal\n";
    open( EVI, ">$outfileEvidence" )
        || die "Can't write to $outfileEvidence -- fatal\n";
...
ASSEMBLY:
    ...
    my ( $seq, $exttig_count, $counter, $extend_bases, $orig_mer, $prevhead,
        $step )
        = ( "", 0, 0, 0, 0, '', 100 );
    while (<IN>) {
        s/\r\n/\n/;
        ...
        if ( /\>(\S+)/ || eof(IN) ) {
            ...
            if ( $seq ne '' ) {
                ...
                ($seq)
                    = doExtension( "3", $orig_mer, $seq, $min_overlap,
                    $min_coverage, $min_base_ratio, $counter )
                    if ( $orig_mer >= $min_overlap );

                my $seqrc = reverseComplement($seq);
                ($seqrc)
                    = doExtension( "5", $orig_mer, $seqrc, $min_overlap,
                    $min_coverage, $min_base_ratio, $counter )
                    if ( $orig_mer >= $min_overlap );

                my $leng       = length($seqrc);
                my $reversetig = reverseComplement($seqrc)
                    ;    ### return orientation of the sequence, as inputted
                if ( $leng > $orig_mer )
                {        #print contigs to file if contig is extended
                    $extend_bases += ( $leng - $orig_mer );
                    printf TIG
                        ">extcontig%i|size%i|prevsize%i|seed:$prevhead\n%s\n",
                        ( $counter, $leng, $orig_mer, $reversetig );
                    $exttig_count++;
                }
                else {
                    printf TIG ">contig%i|size%i|seed:$prevhead\n%s\n",
                        ( $counter, $leng, $reversetig )
    ...
    print SUMFILE
        "\tNumber of unmapped reads used for contig extension = $countReads\n";
    ...
    if ($@) {
        my $message = $@;
        &printMessage( "\nSomething went wrong running $0 "
                . getDate()
                . "\n$message\n" );
    }
    close EVI;
...
sub FormatContigs {
    &printMessage( "\n=>"
            . getDate()
            . ": Storing contigs to format for scaffolding\n" );
    ...
    while (<IN>) {
        ...
        if ( /\>(\S+)/ || eof(IN) ) {
            ...
            { #if length of contig meets the minimum contig length (-z option)
                if ( ++$counter == $step ) {
                    CounterPrint($counter);
                    $step = $step + 100;
                }
...
###EXTEND CONTIGS
sub doExtension {
    my ( $direction, $orig_mer, $seq, $min_overlap, $min_coverage,
        $min_base_ratio, $tig_count )
        = @_;
...
#get one basepair overhang of the contig and determine how many times each nucleotide occurs
        ...
        my $coverage
            ...
            + $overhang->{'T'};
        print EVI
            "\tdir$direction: Total:$coverage\tA:$overhang->{'A'}\tT:$overhang->{'T'}\tG:$overhang->{'G'}\tC:$overhang->{'C'}";
...
#get the two best nucleotides, and see if the best nucleotide meets the criteria (above minimal coverage and below minimum base ratio between the two best bases
    BASE:
        foreach my $bz ( sort { $overhang->{$b} <=> $overhang->{$a} }
            keys %$overhang )
        {
            if ( $ct_dna == 1 )
            {    ## the two most abundant bases at that position
                my $bestcoverage
                    = $overhang->{$previous_bz} + $overhang->{$bz};
                if ( $overhang->{$previous_bz} < $min_coverage )
                { #does the best nucleotide occur more than the minimum coverage
                    $extended = 0;
                    print EVI " => low coverage\n";
                    last CONSENSUS;
                }
                if ( ( $overhang->{$previous_bz} / $bestcoverage )
                    >= $min_base_ratio )
                {    #determine the ratio between the two best bases
...
#for each readfile, map the reads to the contigs and obtain unmapped reads with function readSam
        foreach my $fname (@filesbowtie) {
            my $procline
                = "$bowtiepath -p 1 -v $gaps -m 1 $base_name/alignoutput/$bowtieout -f $fname --quiet --refidx -S --sam-nohead|";
...
#for each readfile, map the reads to the contigs and obtain unmapped reads with function readSam
        foreach my $fname (@filesbwa) {
            my $bwaoutputaln
                = "$base_name/alignoutput/$base_name.extension.$ctlib.bwa";
            my $procline
