...
my ( $library, $aligner, $fileA, $fileB, $insert_size, $insert_stdev,
    $reverse, $libResHash );
my ( $prevlibrary, $ctlib, $libcount ) = ( "", 0, 0 );
&printMessage( "\n=>"
        . getDate()
        . ": Reading, filtering and converting input sequences of library file initiated\n"
);

while (<FILELIB>) {
    chomp;
    (   $library, $aligner, $fileA, $fileB, $insert_size, $insert_stdev,
        $reverse
    ) = split( /\s+/, $_ );
    next if ( $library eq "" );
...
#Process multiple files at the same time if multithreaded option is set (-T parameter larger than 1)
    if ( $aligner ne "TAB" ) {
        my $thr
            = threads->create( \&readUnpairedFile, $fileA, $fname, $ctlib )
            if ( $library eq "unpaired" );
        my $thr = threads->create( \&generateInputFiles,
            $library, $fileA, $fileB, $reverse, $fname, $ctlib, $aligner )
...
foreach my $libs (@libraries) {
    ...
    print SUMFILE
        "$seplines\tTotal inserted reads = $totcounter \n$seplines\n"
        if ( $libs eq "unpaired" );
    print SUMFILE
        "$seplines\tTotal inserted pairs = $totcounter \n$seplines\n"
        if ( $libs ne "unpaired" );
}
close FILELIB;
close SUMFILE;
...
sub generateInputFiles {
    ...
    my ( $counterext, $Ncount, $countsinglet, $fastq, $step, $out )
        = ( 0, 0, 0, 0, 1000000, "" );

    #check if file is zipped, otherwise unzip. C
    if ( $fileA =~ /\.gz$/ ) {
        open( TEST, "gunzip -c  $fileA |" )
            || die "can't open .gz $fileB with gunzip";
    }
    else {
        open( TEST, "< $fileA" );
    }
    ...
    if ( $cthead != $ctlines ) {
        $out
            = getFastAMulti( $fname, $fileA, $fileB, $lib, $base_name,
            $libct )
            if ( !$fastq );
        $out
            = getFastQMulti( $fname, $fileA, $fileB, $lib, $base_name,
            $libct )
            if ($fastq);
    }
    else {
        $out = getFastXSingle( $fname, $fileA, $fileB, $lib, $base_name,
            $libct, $fastq );
    }
    return $out;
}
...
sub getFastXSingle {
    ...
    my ( $seq1, $seq2, $Ncount, $countsinglet, $step, $countSub )
        = ( "", "", 0, 0, 1000000, 1 );
    if ( $fileA =~ /\.gz$/ ) {
        open( FILEA, "gunzip -c  $fileA |" )
            || die "can't open .gz $fileA with gunzip";
    }
    else {
        open( FILEA, "< $fileA" );
    }
    if ( $fileB =~ /\.gz$/ ) {
        open( FILEB, "gunzip -c  $fileB |" )
            || die "can't open .gz $fileB with gunzip";
    }
    else {
        open( FILEB, "< $fileB" );
    }
    open( OUTSINGLEFILE, ">$fname.1.fa" )
        || die "Can't write to single file $fname.1.fa -- fatal\n";
    CounterPrint(
        "Reading single line read-pairs $lib.$libct @ $countsinglet       ");
    while (<FILEA>) {
        <FILEB>;
        $seq1 = <FILEA>;
        $seq2 = <FILEB>;
        ...
        if ( ++$countsinglet == $step ) {
            CounterPrint(
                "Reading read-pairs $lib.$libct @ $countsinglet         ");
            $step = $step + 1000000;
            close OUTSINGLEFILE;
            $countSub++;
            open( OUTSINGLEFILE, ">$fname.$countSub.fa" )
                || die
                "Can't write to single file $fname.$countSub.fa -- fatal\n";
        }
...
sub getFastAMulti {
    ...
    my ($countSub,     $seq1, $seq2, $line1, $Ncount,
        $countsinglet, $step, $res1, $res2
    ) = ( 1, "", "", 0, 0, 1000000 );
    if ( $fileA =~ /\.gz$/ ) {
        open( FILEA, "gunzip -c  $fileA |" )
            || die "can't open .gz $fileA with gunzip";
    ...
    open( OUTSINGLEFILE, ">$fname.1.fa" )
        || die "Can't write to single file $fname.1.fa -- fatal\n";
    CounterPrint(
        "Reading multiline read-pairs $lib.$libct @ $countsinglet         ");
    while ( $line1 = <FILEA> ) {
        chomp $line1;
        if ( $line1 =~ /^>/ || eof(FILEA) ) {
...
sub readUnpairedFile {
    ...
    if ( $file =~ /\.gz$/ ) {
        open( INUNPAIRED, "gunzip -c  $file |" )
            || die "can't open .gz $file with gunzip";
    }
    else {
    ...
    while (<INUNPAIRED>) {
        ...
        if ( ++$counter == $step ) {
            CounterPrint(
                "Reading unpaired reads @ $counter                     ");
            $step = $step + 2000000;
            close OUTFILEExt;
            $countSub++;
            open( OUTFILEExt, ">$fname.$countSub.fa" )
                || die
                "Can't write to single file $fname.$countSub.fa -- fatal\n";
        }
    }
