...
my $verbose        = $ARGV[4];
my $library        = $ARGV[5]; #
my @libraries      = split( /\|/, $library );
   $library        = $libraries[-1];
my $last_library   = $libraries[-1];
my $insert_size    = $ARGV[6]; #
my @insert_sizes   = split( /\|/, $insert_size );
   $insert_size    = $insert_sizes[-1];
...
my %ori_hash;
my %tabfile_hash;
for ( my $i = 0; $i <= $#libraries; $i = $i + 1 ){
    $insert_size_hash{$libraries[$i]} = $insert_sizes[$i];
    $ori_hash{$libraries[$i]} = $oris[$i];
    $tabfile_hash{$libraries[$i]} = $tabfiles[$i];
}


my ( $low_iz, $up_iz )
    = ( $insert_size + $min_allowed, $insert_size - $min_allowed ); # assume that the insert sizes are in an ascending order
...
my $distribution
    = "$base_name/pairinfo/"
    . $base_name
    . ".$library.pairing_distribution.csv";

my ($total_for_median, $ct_illogical,     $ct_ok_contig,
    $ct_ok_pairs,      $ct_problem_pairs, $ct_iz_issues
) = ( 0, 0, 0, 0, 0, 0 );
...
if ( !$tab ) { 
    ...
    if ( $#filesbowtie >= 0 ) {
        ...
        system(
            "$bowbuildpath $contigFile $base_name/alignoutput/$bowtieout --quiet --noref"
            ) == 0
        ...
        foreach my $fname (@filesbowtie) {
            my $procline
                = "$bowtiepath -p 1 -v $gaps -m 1 $base_name/alignoutput/$bowtieout --suppress 6,7 -f $fname --quiet --refidx |";
    ...
    if ( $#filesbwa >= 0 || $#filesbwasw >= 0 ) {
        ...
        system(
            "$bwapath index -a $index $contigFile -p $base_name/alignoutput/$bwaout"
            ) == 0
        ...
        foreach my $fname (@filesbwa) {
            my $bwaoutputaln
                = "$base_name/alignoutput/$base_name.$library.$ctlib.bwa";
            my $procline
                = "$bwapath aln $base_name/alignoutput/$bwaout $fname > $bwaoutputaln"
                ;                        # >/dev/null 2>&1";
            my $samseline
                = "$bwapath samse $base_name/alignoutput/$bwaout $bwaoutputaln $fname |";
        ...
        foreach my $fname (@filesbwasw) {
            my $procline
                = "$bwapath bwasw $base_name/alignoutput/$bwaout $fname |";
    ...
    print SUMFILE
        "$seplines\tNumber of single reads found on contigs = $totalReads\n";
    my $read_number_message
        = "\tNumber of read-pairs used for pairing contigs / total pairs = $totalPairsUsed / $totalPairsFound\n";
...
#-------------------------------------------------BUILDING SCAFFOLDS
&buildScaffolds( $pair, $tig_length, $verbose, $scaffold );
( $pair, $tig_length ) = ( '', '' );
undef $pair;
undef $tig_length;
...
sub parseTabFile {
    ...
    foreach $library (@libraries){
        my $tabfile = $tabfile_hash{$library};
        open( TAB, "$tabfile" )
            || die "Can't open $tabfile for reading -- fatal\n";
    ...
    #check if the contig in the tab file is also present in the inserted contig fasta file
            ...
            if ( !defined( $tigHash->{$tig2} ) ) {
                die
                    "ERROR: could not find an header containing $tig2 at line number $ct_both. Exit...\n";
            }
            ...
            if ( $prev_evidence ne '' ) {
                ...
                if ( $tigOnScafHash->{$ctg1}{'direction'} eq "r" ) {
                    my $tmp_start
                        = (   $tigOnScafHash->{$ctg1}{'end'}
                            + $tigOnScafHash->{$ctg1}{'begin'} )
                        - $start1;
                    my $tmp_end
                        = (   $tigOnScafHash->{$ctg1}{'end'}
                            + $tigOnScafHash->{$ctg1}{'begin'} )
                ...
                if ( $tigOnScafHash->{$ctg2}{'direction'} eq "r" ) {
                    my $tmp_start
                        = (   $tigOnScafHash->{$ctg2}{'end'}
                            + $tigOnScafHash->{$ctg2}{'begin'} )
                        - $start2;
                    my $tmp_end
                        = (   $tigOnScafHash->{$ctg2}{'end'}
                            + $tigOnScafHash->{$ctg2}{'begin'} )
                ...
                if ( $start1 < $up_iz
                    || ( $end1 > ( $tig_length->{$ctg1} - $up_iz ) ) )
                {
                    $track1 = "$ctg1" . "|$start1" . "|$end1" . "|$library";
                }
                if ( $start2 < $up_iz
                    || ( $end2 > ( $tig_length->{$ctg2} - $up_iz ) ) )
                {
                    $track2 = "$ctg2" . "|$start2" . "|$end2";
                }
            }
            else
            {  #if it is the first library, just use the positions in the TAB file
                if ( $start1 < $up_iz
                    || ( $end1 > ( $tig_length->{$ctg1} - $up_iz ) ) )
                {
                    $track1 = "$ctg1" . "|$start1" . "|$end1" . "|$library";
            ...
            pairContigs( $track1, $track2, "seq$ct_both.1", "seq$ct_both.2" )
                if ( $track1 ne "" && $track2 ne "" );
        }
    }
    CounterPrint("                ");
}
...
sub processContig {
    ...
    open( OUT, ">$contigfilesub" )
        || die "can't write to $contigfilesub -- fatal\n";
    my ( $seq, $counter ) = ( '', 0 );
    while (<IN>) {
        chomp;
        my $line = $_;
...
sub determineRepeats {
    ...
    foreach my $tig ( sort { $tig_length->{$b} <=> $tig_length->{$a} }
        keys %$tig_length )
    {
        for ( my $i = 0; $i < 2; $i++ ) {
            my $dtig = "r" . $tig;
          ...
          #Go through each contig pair and get the number of links and gapsize
            foreach my $match (
                sort { $list->{$b}{'links'} <=> $list->{$a}{'links'} }
                keys %$list )
            {
                my $matchnum = $1 if ( $match =~ /[fr](\w+)/ );
                print TMP
                    "$dtig has $list->{$match}{'links'} links with $match and gap of "
                    . int(
                    $list->{$match}{'gaps'} / $list->{$match}{'links'} )
                    . " bases\n"
                    if ( $list->{$match}{'links'} >= $min_links );
                if (   $list->{$match}{'links'} >= $min_links
                    && !defined $seen_it->{$matchnum}
                    && $ct < 2 )
                {
                    $ct++;
...
#$matchhash->{$match}{'ratio'} = $list->{$match}{'gaps'}/$list->{$match}{'links'};
                    $matchhash->{$match}{'ratio'}
                        = &getMidGap( $dtig, $match );
                    $seen_it->{$matchnum}++;
                }
            }
            my @arraymatch;
            foreach my $ratiosort (
                sort {
                    $matchhash->{$a}{'ratio'} <=> $matchhash->{$b}{'ratio'}
                } keys %$matchhash
                )
            {
...
#only determine if contig is a repeat if it has more than 1 link with other contigs
            if ( $nummatch > 0 ) {
                ...
                if ( $listmatch->{ $arraymatch[1] }{'links'} >= $min_links ) {
                    ...
                    $pair = establishLink( $arraymatch[0], $arraymatch[1],
                        $pair );
                }
                else
                { #otherwise, the contig has multiple links and is likely a repeat
                    my @linkmatch;
                    foreach my $linksort (
                        sort {
                            $matchhash->{$b}{'links'}
...
#If either one of the two ratio's is above the user-defined (-a) ratio, the original contig is treated as a repeat
       ...
       #estimate the ratio of the links of the two best contig pairs (ratio 1)
                    ...
                    my $ratio1 = $link1 / $link2
                        ; ## relative ratio of the two most abundant contig pairs
                    $ratio1 = sprintf( "%.2f", $ratio1 );
                    $first = $linkmatch[0];

#estimate the number of links per gap for the two best contig pairs and divide them (ratio 2)
                    my $gapPerSpace1 = estimateLinksPerGap(
                        $matchhash,   $linkmatch[0], $tig_length,
                        $insert_size
                    );
                    my $gapPerSpace2 = estimateLinksPerGap(
                        $matchhash,   $linkmatch[1], $tig_length,
                        $insert_size
                    );
                    ...
                    if (   $ratio2 >= $max_link_ratio
                        || $ratio1 >= $max_link_ratio
                        || $first ne $second )
                    {
                        foreach my $linksort (
                            sort {
                                $list->{$b}{'links'} <=> $list->{$a}{'links'}
                            } keys %$list
                            )
                        {
                    ...
                    { #otherwise, establish the link between the most likely contig pair
                        foreach my $linksort (
                            sort {
                                $list->{$b}{'links'} <=> $list->{$a}{'links'}
                            } keys %$list
...
sub estimateLinksPerGap {
    my ( $linkhash, $tig1, $length_hash, $insert_size ) = @_;
...
sub buildScaffolds {
    my ( $pair, $tig_length, $verbose, $scaffold ) = @_;
...
#determine the repeats and remove any link if contig is a repeat
    ...
    open( TMP,
              ">$base_name/intermediate_results/$base_name"
            . "_$library.foundlinks.txt" )
        || die "Can't write to $base_name/intermediate_results/$base_name"
        . "_$library.foundlinks.txt -- fatal\n";
    $repeathash = determineRepeats( $tig_length, $repeathash );
    close TMP;
    open( REPEAT,
              ">$base_name/intermediate_results/$base_name"
            . "_$library.repeats.txt" )
        || die "Can't write to $base_name/intermediate_results/$base_name"
        . "_$library.repeats.txt -- fatal\n";
    foreach my $rep ( sort keys %$repeathash ) {
        ...
        foreach my $rep2 ( sort keys %$ls ) {
            if ( $pair->{$rep}{$rep2}{'links'} >= $min_links ) {
                $repline
                    .= "\twith $rep2 (links = $pair->{$rep}{$rep2}{'links'})\n";
                $num_match++;
            }
        ...
        if ( $num_match > 1 ) {
            $numrepeat++;
            print REPEAT
                "Contig $rep (size = $tig_length->{$tig}) has $num_match multiple links;\n";
...
SEED:
        ...
        keys %$tig_length )
    {
        ...
        { ##should prevent re-using a contig as seed if it's already been incorporated into a scaffold
            ...
            ( $total, $chainright, $seen_start )
                = &computeLayout( "R", $chainright, $ftig, $pair,
                $tig_length, $total, $seen_start, $tig );
            ( $total, $chainleft, $seen_start )
                = &computeLayout( "L", $chainleft, $rtig, $pair, $tig_length,
                $total, $seen_start, $tig );
...
sub computeLayout {
    my ( $ext, $chain, $tig, $pair, $tig_length, $total, $seen_start,
        $orig_tig_number )
        = @_;
    my $orig_tig  = $tig;
    my $extension = 1;
EXTENSION:
    while ($extension) {
        ...
        {    #if already seen in scaffold, do not use it again
            $seen_start->{$tnum}++ if ( $tnumf ne $orig_tig );
            my $list = $pair->{$tig};
            my $matchhash;
            my ( $match1, $link1, $gaps1, $match2, $link2, $gaps2, $cntloop,
                $countmatches )
                = ( "", 0, 0, "", 0, 0, 0, 0 );
            my $ct = 0;
        LINK:
                ...
                keys %$list )
            {
                my $matchnum = $1 if ( $match =~ /[fr](\w+)/ );
                if (   $list->{$match}{'links'} >= $min_links
                    && !defined $seen_start->{$matchnum}
                    && $matchnum ne $orig_tig_number
                    && $ct < 2 )
                {
                    $ct++;
                    $matchhash->{$match}{'links'} = $list->{$match}{'links'};
...
#$matchhash->{$match}{'ratio'} = $list->{$match}{'gaps'}/$list->{$match}{'links'};
            ...
            if ( $countmatches > 1 ) {
                ...
                for ( my $i = 0; $i <= $nummatch && $foundlinks < 1; $i++ ) {
                    ...
                    for (
                        my $j = $i + 1;
                        $j <= $nummatch && $foundlinks < 1;
                        $j++
                        )
                    {
                        my $linkmatch
                            = $listmatch->{ $arraymatch[$j] }{'links'};
                        $foundlinks = 1 if ( !( $linkmatch >= $min_links ) );
                    }
                }
                my $tignum = $1 if ( $arraymatch[$nummatch] =~ /[fr](\w+)/ );
                $countmatches = 0
                    if ( !$foundlinks && defined $seen_start->{$tignum} );
            }
            if ( $foundlinks && $countmatches > 1 ) {
                ...
                my $link1 = $matchhash->{ $linkmatch[1] }{'links'};
                my $link2 = $matchhash->{ $linkmatch[0] }{'links'};
                my $ratio = $link1 / $link2
                    ;  ## relative ratio of the two most abundant contig pairs
                $ratio = sprintf( "%.2f", $ratio );

                if ( $ratio <= $max_link_ratio ) {
                    foreach my $mat ( keys %$matchhash ) {
                        delete $matchhash->{$mat}
                            if ( $mat ne $linkmatch[0] );
                    }
                    $foundlinks   = 0;
                    $countmatches = 1;
                }
                    ...
                    )
                {
                    if ( $tig ne $incl_matches ) {
                      ...
                      #my $mean2 = $listmatch->{$incl_matches}{'gaps'}/$link2;
                        my $median = &getMidGap( $tig, $incl_matches );
                        my ( $perc10, $perc90 )
                            = &getGapperc10perc90( $tig, $incl_matches );

                        $seen_start->{$tempnum}++
                            if ( $nummatch < $countmatches );

                        ( $chain, $total, $tig ) = &getChain(
                            $chain,        $ext,     $link2,
                            $median,       $perc10,  $perc90,
                            $incl_matches, $tempnum, $ratio,
                            $tig_length,   $total
                        );
...
sub getChain {
    my ($chain, $ext,     $link,  $median,     $perc10, $perc90,
        $match, $tempnum, $ratio, $tig_length, $total
    ) = @_;
    my $tig = $match;
    if ( $ext eq "R" ) {
        $chain
        ...
        my $temp_match = "";
        if   ( $match =~ /^r(\d+)/ ) { $temp_match = "f" . $1; }
        else                         { $temp_match = "r" . $1; }
        $chain
            = $temp_match . "z"
            . $tig_length->{$tempnum} . "k"
            . $link . "a"
            . $ratio . "m"
            . int($median) . "p"
            . int($perc10) . "P"
...
sub pairContigs {
    ...
    my ( $tig_a, $A_start, $A_end, $library ) = split( /\|/, $trackA );
    my ( $tig_b, $B_start, $B_end ) = split( /\|/, $trackB );
    my ( $ori_1, $ori_2 ) = split( //, $ori_hash{$library} );
    if ( $ori_1 eq "R" ) {
    ...
    if ( $tig_a != $tig_b ) {    ####paired reads located on <> contigs
        ...
        if ( $A_start < $A_end ) {
            if ( $B_end < $B_start ) {  ####-> <- :::  A-> <-B  /  rB -> <- rA
                my $d = &getDistance( $insert_size_hash{$library}, $A_length, $A_start,
                    $B_start );
                print
                    "A-> <-B  WITH $tig_a -> <- $tig_b GAP $d A=$A_length ($A_start-$A_end) B=$B_length ($B_start-$B_end) Alen, Astart,Bstart\n"
                    if ($verbose);
                if ( $d >= $min_allowed ) {
                    ...
                    $ct_problem_pairs++;
                    print PET
                        "Pairs unsatisfied in distance within a contig pair.  A-> <-B  WITH tig#$tig_a -> $d <- tig#$tig_b, A=$A_length nt (start:$A_start, end:$A_end) B=$B_length nt (start:$B_start, end:$B_end) CALCULATED DISTANCE APART: $d < $min_allowed\n";
                }
            }
            else {    #### -> -> ::: A-> <-rB  / B-> <-rA
                ...
                my $d = &getDistance( $insert_size_hash{$library}, $A_length, $A_start,
                    $rB_start );
                print
                    "A-> <-rB  WITH $tig_a -> <- r.$tig_b GAP $d A=$A_length ($A_start-$A_end) B=$B_length ($B_start-$B_end) Alen,Astart,rBstart\n"
                    if ($verbose);
                if ( $d >= $min_allowed ) {
                    ...
                    $ct_problem_pairs++;
                    print PET
                        "Pairs unsatisfied in distance within a contig pair.  A-> <-rB  WITH tig#$tig_a -> $d <- tig#r.$tig_b, A=$A_length  nt (start:$A_start, end:$A_end) B=$B_length nt (start:$B_start, end:$B_end) CALCULATED DISTANCE APART: $d < $min_allowed\n";
                }
            }
        }
        else {
            if ( $B_end > $B_start ) {    ####<-  -> ::: B-> <-A / rA -> <- rB
                my $d = &getDistance( $insert_size_hash{$library}, $B_length, $B_start,
                    $A_start );
                print
                    "B-> <-A  WITH $tig_b -> <- $tig_a GAP $d A=$A_length ($A_start-$A_end) B=$B_length ($B_start-$B_end) Blen,Bstart,Astart\n"
                    if ($verbose);
                if ( $d >= $min_allowed ) {
                    ...
                    $ct_problem_pairs++;
                    print PET
                        "Pairs unsatisfied in distance within a contig pair.  B-> <-A  WITH tig#$tig_b -> $d <- tig#$tig_a, B=$B_length nt (start:$B_start, end:$B_end) A=$A_length nt (start:$A_start, end:$A_end) CALCULATED DISTANCE APART: $d < $min_allowed\n";
                }
            }
            else {    ####<- <-  :::  rB-> <-A / rA-> <-B
                my $rB_start = $B_length - $B_start;
                my $d = &getDistance( $insert_size_hash{$library}, $B_length, $rB_start,
                    $A_start );
                print
                    "rB-> <-A WITH r.$tig_b -> <- $tig_a GAP $d A=$A_length ($A_start-$A_end) B=$B_length ($B_start-$B_end) Blen,rBstart,Astart\n"
                    if ($verbose);
                if ( $d >= $min_allowed ) {
                    ...
                    $ct_problem_pairs++;
                    print PET
                        "Pairs unsatisfied in distance within a contig pair.  rB-> <-A WITH tig#r.$tig_b -> $d <- tig#$tig_a, B=$B_length nt (start:$B_start, end:$B_end) A=$A_length nt (start:$A_start, end:$A_end) CALCULATED DISTANCE APART: $d < $min_allowed\n";
                }
            }
        }
    }
    ...
    { ###Clone, paired reads located on the same contig -- could be used to investigate misassemblies
        print
            "Pair ($read_a and $read_b) located on same contig $tig_a ($A_length nt)\n"
            if ($verbose);
        my $pet_size = 0;
        if ($last_library == $library){
            if (   $A_start > $B_start
                && ( $B_start < $B_end )
                && ( $A_start > $A_end ) )
            {    # B --> <-- A
                ...
                if ( $pet_size >= $low_iz && $pet_size <= $up_iz ) {
                    ...
                    print PET
                        "Pairs unsatisfied in distance within a contig.  Pair ($read_a - $read_b) on contig $tig_a ($A_length nt) Astart:$A_start Aend:$A_end Bstart:$B_start Bend:$B_end CALCULATED DISTANCE APART: $pet_size\n";
                    $ct_iz_issues++;
                }
            }
            ...
            {    # A --> <-- B
                ...
                $ct_illogical++;
                print PET
                    "Pairs unsatisfied in pairing logic within a contig.  Pair ($read_a - $read_b) on contig $tig_a ($A_length nt) Astart:$A_start Aend:$A_end Bstart:$B_start Bend:$B_end\n";
            }
        }
    }
}
...
sub printResultsPairing {
    print PET
        "------------- Putative issues with contig pairing - Summary  ----------------\n";
    foreach
        my $err_pair ( sort { $err->{$b}{'links'} <=> $err->{$a}{'links'} }
        keys %$err )
    {
        my $mean_iz = 0;
        $mean_iz = $err->{$err_pair}{'gaps'} / $err->{$err_pair}{'links'}
            if ( $err->{$err_pair}{'links'} );
        print PET
            "Pair $err_pair has $err->{$err_pair}{'links'} links and mean distance = $mean_iz\n";
    }
    close PET;
    ...
    open( CSV, ">$distribution" )
        || die "Can't open $distribution for writing -- fatal";
    my ( $total_is, $overal_is, $median_ins, $stdev, $record, $sumX, $sumX2 )
        = ( 0, 0, 0, 0, 0, 0, 0 );
    ...
    foreach my $is ( sort { $a <=> $b } keys %$track_insert ) {
        for ( my $i = 0; $i < $track_insert->{$is}; $i++ ) {
            ...
            $sumX2 += ( $is * $is );
            $median_ins = $is
                if ( $record >= $median_bin && $median_ins == 0 );
        }
    ...
    if ( $sumX > 0 && $record > 0 ) {
        ...
        $sigma    = sprintf( "%.2f",
            sqrt( $sumX2 / $record - $mean_ins * $mean_ins ) );
    }
    close CSV;

    print SUMFILE "READ PAIRS STATS:\n";
    print SUMFILE
        "\tAssembled pairs: $totalPairsUsed ($totalPairsUsed_reads sequences)\n";
    print SUMFILE
        "\t\tSatisfied in distance/logic within contigs (i.e. -> <-, distance on target: $insert_size +/$min_allowed): $ct_ok_contig\n";
    print SUMFILE
        "\t\tUnsatisfied in distance within contigs (i.e. distance out-of-bounds): $ct_iz_issues\n";
    print SUMFILE
        "\t\tUnsatisfied pairing logic within contigs (i.e. illogical pairing ->->, <-<- or <-->): $ct_illogical\n";
    print SUMFILE "\t\t---\n";
    print SUMFILE
        "\t\tSatisfied in distance/logic within a given contig pair (pre-scaffold): $ct_ok_pairs\n";
    print SUMFILE
        "\t\tUnsatisfied in distance within a given contig pair (i.e. calculated distances out-of-bounds): $ct_problem_pairs\n";
    print SUMFILE "\t\t---\n";
    print SUMFILE
        "\tTotal satisfied: $satisfied\tunsatisfied: $unsatisfied\n\n";
    print SUMFILE
...
sub readBowtie {
    my ($input) = @_;
    my ( $counter, $pair_found, $line, $prevline, $prevread )
        = ( 0, 0, "", "", "" );
...
sub StoreResultsBWA {
    ...
    if ( --$result[3] > $up_iz && $tig_length->{ $result[2] } > $lenlimit ) {
        $result[3] = (
            $tig_length->{ $result[2] } - ( $subContigLim - $result[3] ) );
    }
    if ( $result[1] & 16 ) {
...
###FUNCTION TO CALCULATE MEDIAN GAP BETWEEN A CONTIG PAIR
sub getMidGap {
    my ( $contigA, $contigB ) = @_;
    my ( $mean_gap, $median_gap, $record ) = ( 0, 0, 0 );
    my $links        = $pair->{$contigA}{$contigB}{"links"};
    my $median_count = int( ( $links + 1 ) / 2 );
    my @gaps         = split( /\|/, $pair->{$contigA}{$contigB}{"gaps"} );
    shift @gaps;
    @gaps = sort { $a <=> $b } @gaps;
    foreach my $gap (@gaps) {
        $record++;
        $mean_gap += $gap;
        $median_gap = $gap
            if ( $record >= $median_count && $median_gap == 0 );
    }
    $mean_gap = int( $mean_gap / $links );
    return $median_gap;
}

sub getGapperc10perc90 {
    my ( $contigA, $contigB ) = @_;
    my ( $perc10_gap, $perc90_gap, $record ) = ( 0, 0, 0 );
    my $links        = $pair->{$contigA}{$contigB}{"links"};
    my $perc10_count = int( ( $links + 1 ) / 10 );
    $perc10_count = 1 if ($perc10_count == 0);
    my $perc90_count = int( $links + 1 ) - $perc10_count;
    my @gaps         = split( /\|/, $pair->{$contigA}{$contigB}{"gaps"} );
    shift @gaps;
    @gaps = sort { $a <=> $b } @gaps;
    ...
    foreach my $gap (@gaps) {
        $record++;
        $perc10_gap = $gap if ( $record == $perc10_count );
        $perc90_gap = $gap if ( $record == $perc90_count );
    }
    return ( $perc10_gap, $perc90_gap );
}
