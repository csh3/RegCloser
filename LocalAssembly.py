...
def generateConsensus(reads, estimates):
    ...
    consensus=[[c] for c in reads[0]['seq']]
    phreds=[[1-10**(-s/10)] for s in reads[0]['phred']]
    readsName=[reads[0]['name']]
    readNum=1
    adjust=10

    query_backup=reads[0]['seq']
    estimate_backup=estimates[0]
    coordinate_backup=len(consensus)
    ...
    for i in range(1,len(reads)):
        ...
        consensusSeq=''
        insertionNum=0
        for bases in consensus:
            count=collections.Counter(bases)
            if ' ' in count:
                count.pop(' ')
        ...
        if score>=20:
            query_backup=query
            estimate_backup=estimate
            coordinate_backup=startPoint+end1+len(query)-end2
            ...
            for j in range(len(cigar_ref)):
                ...
                for k in range(c_r[1]-c_r[0]):
                    consensus[startPoint+start1-1+c_r[0]+k].append(query[start2-1+c_q[0]+k])
                    phreds[startPoint+start1-1+c_r[0]+k].append(1-10**(-phred[start2-1+c_q[0]+k]/10))
            ...
            if hoC>=hoQ:
                for j in range(hoQ):
                    consensus[len(consensusSeq)-hoC+j].append(query[len(query)-hoQ+j])
                    phreds[len(consensusSeq)-hoC+j].append(1-10**(-phred[len(query)-hoQ+j]/10))
                for j in range(hoC-hoQ):
                    consensus[len(consensusSeq)-hoC+hoQ+j].append(' ')
                    phreds[len(consensusSeq)-hoC+hoQ+j].append(' ')
            else:
                for j in range(hoC):
                    consensus[len(consensusSeq)-hoC+j].append(query[len(query)-hoQ+j])
                    phreds[len(consensusSeq)-hoC+j].append(1-10**(-phred[len(query)-hoQ+j]/10))
                for j in range(hoC,hoQ):
                    insert=[' ']*readNum+[query[len(query)-hoQ+j]]
                    consensus.append(insert)
                    insert=[' ']*readNum+[1-10**(-phred[len(query)-hoQ+j]/10)]
                    phreds.append(insert)
            ...
            if hoQ<=hoC:
                ...
                for j in range(hoC-hoQ,hoC):
                    consensus[j].append(query[j-(hoC-hoQ)])
                    phreds[j].append(1-10**(-phred[j-(hoC-hoQ)]/10))
            else:
                for j in range(hoC):
                    consensus[j].append(query[hoQ-hoC+j])
                    phreds[j].append(1-10**(-phred[hoQ-hoC+j]/10))
            ...
            for j in range(len(cigar_ref)-1):
                ...
                if right-left>0:
                    for k in range(left,right):
                        consensus[startPoint+start1-1+k].append('-')
                        phreds[startPoint+start1-1+k].append('-')
                ...
                if cigar_ref[j][1]==cigar_ref[j+1][0]:
                    for k in range(cigar_query[j+1][0]-cigar_query[j][1]):
                        ...
                        for l in range(readNum):
                            if consensus[site-1][l]!=' ' and consensus[site][l]!=' ':
                                insert.append('-')
                            else:
                                insert.append(' ')
                        consensus.insert(site,insert+[query[start2-1+cigar_query[j][1]+k]])
                        phreds.insert(site,insert+[1-10**(-phred[start2-1+cigar_query[j][1]+k]/10)])
    ...
    while(True):
        ...
        for column in consensus:
            ...
            for i in range(len(column)):
                if column[i]!=base and column[i]!=' ':
                    error_reads.append(i)
            if sum(count.values())==1:
                for i in range(len(column)):
                    if column[i]!=' ':
                        essential_reads.append(i)
        if error_reads==[]:
            break
        else:
            ...
            if count_error_reads=={}:
                ...
                if count_error_reads[remove_read]<=5:
                    ...
                    for i in range(len(consensus)):
                        consensus[i].pop(remove_read)
                        phreds[i].pop(remove_read)
            ...
            if k==0:
                ...
                if 'RL' in Gap['gapSequence']:
                    ...
                    if filledLen > 0:
                        ...
                        writeSequence(gapFile, Gap['gapSequence']['filledSeq'])
                        gapFile.write('+\n')
                        writeSequence(gapFile, Gap['gapSequence']['filledPhred'])
                    else:
