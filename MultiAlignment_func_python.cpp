...
struct Result_merge* MultiOptimal_merge_alignment (char *seq_a, char *seq_b, 
	...
	for(int k = 0; k < multi; k++){
		...
		traceback_a="";
		traceback_b="";
		while(trace[current_i][current_j]!=3 and current_i>0 and current_j>0){
		...
		temp[1] = MH[k][start_i-1][start_j]-delta;
		temp[2] = MH[k][start_i][start_j-1]-delta;
		temp[3] = 0;
		double tmax = temp[1];
		trace[start_i][start_j] = 1;
		for(int k = 2; k < 4; ++k){
			if(temp[k] > tmax){
				tmax = temp[k];
