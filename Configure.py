...
library_name	filename1	filename2	library-mean	library-std		direction	extension
(lib_name 		file_name_1	file_name_2	    mean	       std		      FR/RF 	  Y/N)
'''
...
with open('scripts/CollectReads.sh','w+') as fout:
	...
	for library in library_info:
		...
		if args.nr:
			fout.write('python %s/CollectReads.py -nr -c %d -l %s -d %s -m %d -s %d -e %s -tl %d -ml %d -mq %d &\n'%(path, args.c, library, library_info[library]['direction'], library_info[library]['mean'], library_info[library]['std'], library_info[library]['extension'], library_info[library]['trimlength'], args.ml, args.mq))
		else:
			fout.write('python %s/CollectReads.py -c %d -l %s -d %s -m %d -s %d -e %s -tl %d -ml %d -mq %d &\n'%(path, args.c, library, library_info[library]['direction'], library_info[library]['mean'], library_info[library]['std'], library_info[library]['extension'], library_info[library]['trimlength'], args.ml, args.mq))
	fout.write('\nwait\n')
